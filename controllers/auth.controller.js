...
exports.adminLogin = asyncHandler(async (req, res) => {
    ...
    const token = jwt.sign({ _id: result._id }, process.env.CLOUDINARY_API_SECRET, { expiresIn: "1d" })
    res.cookie("todo-admin", token, {
        maxAge: 1000 * 60 * 60 * 24,
        httpOnly: true,
        secure: process.env.NODE_ENV === "prod" ? true : false //keep false for local and true in live
    })
    res.json({
